...
int main(int argc, char const *argv[]) {
    ...
    // CSV output file
    ofstream csv_file("calibration_output_noiseRed.csv");
    ...
    for (int event = 0; event < max_entries; ++event) {
        ...
        for (int k = 0; k < calo_nohits; ++k) {
            ...
            // Skip om_num greater than 520
            if (om_num > 520) {
                continue;  // Skip this OM number if it exceeds 520
            }
           ...
           std::cout << "OM range: " << om_min << " to " << om_max << std::endl;
    ...
    for (const auto &om_entry : histograms) {
        ...
        for (const auto &cell_entry : om_entry.second) {
            ...
            // // Save problematic fits
            // if (red_chi2 > 40) {
            // TCanvas *c1 = new TCanvas(Form("c_om%d_cell%d", om_num, mem_cell_no),
            //                            Form("Fit: OM %d, Cell %d", om_num, mem_cell_no), 800, 600);
            // hist->Draw();
            // gaus_fit->Draw("same");
            // c1->SaveAs(Form("high_redchi2_fits/fit_om%d_cell%d_noise_red.png", om_num, mem_cell_no));
            // delete c1;
            // }
